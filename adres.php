...
Ulica='".$wart3."',
Nr_domu='".$wart4."'
WHERE Id_adres=".$wart0 ;
}	
@mysqli_query($polaczenie, $q02) ;	

$q03='DELETE FROM adres WHERE Id_adres='.$ids ;
...
<div id='right2'>
    ...
    <?php
        ...
        echo ("<table class='table-fill' id='t12'> <tr>");
        foreach($row as $key => $value)
        {echo("<th class='text-left'> <a href='adres.php?sort=".((@$_GET['sort']==-$i)?$i:-$i)."'>".$key."</a> </th>");
        $tabs[$i]=$key;
		$tabs[-$i]=$key;
		$i++;
		} 
        ...
        if ($result = $polaczenie->query($q02)) { 
            ...
            while ($row1 = $result->fetch_assoc()) {
                ...
                echo("<td>  <a href='adres.php?ids=".$a."' > Delete </a> </td>");
				echo("</tr>");
				echo("</form>")	;
                $a=-1;
            }    
