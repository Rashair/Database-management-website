...
PESEL='".$wart3."',
Plec='".$wart4."',
Data_urodzenia='".$wart5."',
Id_adresu=".$wart6.",
Id_stanowiska=".$wart7."
WHERE Id_osoba=".$wart0 ;
}	
@mysqli_query($polaczenie, $q02) ;
	
$q03='DELETE FROM osoba WHERE Id_osoba='.$ids ;
...
<div id='right2'>
    ...
    <?php
        ...
        echo ("<table class='table-fill' id='t11'> <tr>");
        foreach($row as $key => $value)
        {echo("<th class='text-left'> <a href='osoba.php?sort=".((@$_GET['sort']==-$i)?$i:-$i)."'>".$key."</a> </th>");
        $tabs[$i]=$key;
		$tabs[-$i]=$key;
		$i++;
		} 
        ...
        if ($result = $polaczenie->query($q02)) { 
           ...
           while ($row1 = $result->fetch_assoc()) {
               ...
               foreach($row1 as $key => $value) {  
                ...
                echo("<td>  <a href='osoba.php?ids=".$a."' > Delete </a> </td>");
				echo("</tr>");
				echo("</form>")	;
                $a=-1;
            }    
